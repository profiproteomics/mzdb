...
@Ignore
class IdentifiedPrecursorsIsotopesTest extends StrictLogging {
  ...
  def testMGFGeneration() = {
    ...
    val lines = Source.fromInputStream(IdentifiedPrecursorsIsotopesTest.this.getClass.getResourceAsStream("/v3.6_list.csv")).getLines().drop(1).toSeq
    ...
    val fw = new BufferedWriter(new FileWriter(new File((new File(mzdbFilePath)).getParentFile, "precursors_isotopes_stats.txt" )))
    ...
    for( (scan, identification) <- identsByScan  ) {
      ...
      var map = precComputer.extractPrecursorIsotopesStats(mzDbReader, spectrumHeader, identification.bestMoz, identification.bestCharge, mzTol)
      ...
      if (identification.worstQuery >= 0) {
        var map = precComputer.extractPrecursorIsotopesStats(mzDbReader, spectrumHeader, identification.worstMoz, identification.worstCharge, mzTol)
        map += ("scan.number" -> scan)
        map += ("scan.rt" -> spectrumHeader.getElutionTime/60.0)
        map += ("ident.status" -> identification.status())
        map += ("ident.duplicated.status" -> identification.duplicatedStatus())
