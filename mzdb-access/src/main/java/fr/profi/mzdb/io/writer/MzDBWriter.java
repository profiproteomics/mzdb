...
public class MzDBWriter {
    ...
    public MzDBWriter(File location, boolean useTmp, MzDBMetaData metaData, BBSizes bbSizes, Boolean isDIA) {
        ...
        finalDBLocation = location;
        tmpDBLocation = useTmp ? new File(location.getAbsolutePath() + ".tmp") : location;
    ...
    public void initialize() throws SQLiteException {
        logger.debug("Initialize Writer for " + tmpDBLocation.getName());
        createConnection();
        if (metaData != null) {
            insertMetaData();
        }
...
//
      ...
      userParam.name match {
        ...
        if (samples.isEmpty()) {
            ...
            if (sourceFiles.isEmpty())
                sampleName = tmpDBLocation.getName().split("\\.")[0];
            else
    ...
    public void close() {
        ...
        if (!finalDBLocation.equals(tmpDBLocation)) {
            tmpDBLocation.renameTo(finalDBLocation);
        }
    ...
    public void insertSpectrum(Spectrum spectrum, SpectrumMetaData metaDataAsText, DataEncoding dataEncoding) throws SQLiteException {
        ...
        try {
            ...
            if (!emptySpectrum) {
                ...
                insertSpectrumToWrite(spectrumToWrite, spectrumToWrite.spectrumId);
            }
