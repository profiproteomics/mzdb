...
public class Peak implements Cloneable, Comparable<Peak> {
   ...
   * @param leftHwhm the left hwhm
   * @param rightHwhm the right hwhm
   * @param lcContext the LC context
   */
   ...
   * Instantiates a new peak.
   *
   * @param mz peak's mass
   * @param intensity peak's intensity
   */
   ...
   * Sets the lc context.
   *
   * @param lcContext the new lc context
   */
  public void setLcContext(ILcContext lcContext) {
    this.lcContext = lcContext;
  }
  
  public boolean isNormalized() { return this._nf > 0; }
  ...
  // TODO: put private (also ins scan headers and other comparators)
	...
	public static Comparator<Peak> intensityComp = new Comparator<Peak>() {
		...
		public int compare(Peak peak0, Peak peak1) {
			if (peak0.intensity < peak1.intensity) return -1;
			else if (Math.abs(peak0.intensity - peak1.intensity) < 1e-6) return 0;
			else return 1;
		}
	};	
