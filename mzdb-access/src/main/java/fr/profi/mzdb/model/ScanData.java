...
public class ScanData {
  ...
  public float[] getRightHwhmList() {
	...
	public boolean isEmpty() {
		return mzList.length==0; //supposing intensityList and others have the same size;
	}
	
	/**
	 * _bin search index to nearest index.
	 *
	 * @param binSearchIndex the bin search index
	 ...
	 * Mz range filter.
	 *
	 * @param mzMin the mz min
	 * @param mzMax the mz max
	...
	public ScanData mzRangeFilter(double mzMin, double mzMax) {
	  ...
	  // If out of bounds => set the last value index as the array last index
		...
		if ( this.leftHwhmList != null ) {
			...
			filteredScanData.rightHwhmList = Arrays.copyOfRange(this.rightHwhmList, firstIdx, exclusiveLastidx);
		}
