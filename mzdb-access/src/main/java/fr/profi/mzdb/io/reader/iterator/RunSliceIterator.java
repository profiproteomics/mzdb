...
public class RunSliceIterator extends AbstractScanSliceIterator implements Iterator<RunSlice>  {
	...
	//private static String sqlQuery = "SELECT * FROM scan ORDER BY run_slice_id";
	//private static string sqlQuery = "SELECT bounding_box.* FROM bounding_box, run_slice WHERE run_slice.ms_level = ? AND bounding_box.id IN (SELECT id from bounding_box_rtree WHERE ORDER BY run_slice.begin_mz"
