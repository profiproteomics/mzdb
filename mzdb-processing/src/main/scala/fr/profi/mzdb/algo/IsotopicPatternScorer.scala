...
object IsotopicPatternScorer extends LazyLogging {
  ...
  /*
  // TODO: remove me => this method seems to not be used
   ...
   */
   private def calcIsotopicPatternHypothesis(
    ...
    this.calcIsotopicPatternHypothesis(
      ...
      charge,
      ppmTol,
      IsotopePatternEstimator.getTheoreticalPattern(mz, charge),
      rank => Math.max(0.01, Math.pow(10, -(2 * rank - 4))) // Math.min(100.0, 0.0001 * Math.pow(10, rank*2))
    )
