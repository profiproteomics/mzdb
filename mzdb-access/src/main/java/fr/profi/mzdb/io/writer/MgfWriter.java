...
public class MgfWriter {
  ...
  /** */
  public enum PrecursorMassStrategy {
    DEFAULT(""),
    REFINED_PWIZ("pwiz refined precursor mz"),
    REFINED_MZDB("mzdb refined precursor mz");
    
    private final String paramName;
    
    PrecursorMassStrategy(String f) {
      this.paramName = f;
    }
  ...
  public MgfWriter(String mzDBFilePath) throws SQLiteException {
    ...
    this.fillTitleByScanId();
    this.logger.debug("length titleByScanId:" + this.titleByScanId.size());
  }
  
  private void fillTitleByScanId() throws SQLiteException {
   ...
   * @throws SQLiteException 
   */
  public void write(String mgfFile, PrecursorMassStrategy pm) 
  ...
  public class MgfHeaderEntry {
    ...
    @Override
    public String toString() {
      StringBuilder sb = new StringBuilder().append(field)
                                            .append(MgfField.EQUAL)
                                            .append(value)
                                            .append(trailer);
  ...
  public void writeScan(PrintWriter mgfWriter, Scan scan, PrecursorMassStrategy pms) {
    //unpack data
    ...
    if (pms == PrecursorMassStrategy.DEFAULT){ 
      
    } else if (pms == PrecursorMassStrategy.REFINED_PWIZ) {
      try {
          precMz = Double.parseDouble(scanHeader.getUserParam(PrecursorMassStrategy.REFINED_PWIZ.getUserParamName())
                                                .getValue());
      } catch (NullPointerException e) {
        this.logger.info("Refined pwiz user param name not found: fall back to default");
      }
    } else if ( pms == PrecursorMassStrategy.REFINED_MZDB) {
      try {
        precMz = Double.parseDouble(scanHeader.getUserParam(PrecursorMassStrategy.REFINED_MZDB.getUserParamName()).getValue());
      } catch (NullPointerException e) {
    ...
    for (int i = 0; i < mzs.length; ++i) {
        ...
        mgfWriter.print(String.format("%1.6f", mz) + "\\s" +  String.format("%f", intensity) + "\\n");
    }
