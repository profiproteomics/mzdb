...
public class ScanHeaderReader extends AbstractMzDbReaderHelper {
  ...
  private ISQLiteRecordExtraction<ScanHeader> _scanHeaderExtractor = new ISQLiteRecordExtraction<ScanHeader>() {
    ...
    public ScanHeader extract(SQLiteRecord record) throws SQLiteException {
      ...
      double precursorMz = 0.0; int precursorCharge = 0;
      if (msLevel == 2) {
      ...
      boolean isHighRes = ( val.equals("true") ) ? true : false;//paramTreeAsStr.contains("FTMS") ? true : false;
  ...
  public ScanHeader[] getScanHeaders() throws SQLiteException {
    ...
    if ( this.entityCache != null && this.entityCache.scanHeaders != null) {
      ...
      new SQLiteQuery(connection,_scanHeaderQueryStr).extractRecords( this._scanHeaderExtractor, scanHeaders );
   ...
   * @throws SQLiteException the sQ lite exception
   */
  /*public HashMap<Integer, ScanHeader> getScanHeaderByInitialId() throws SQLiteException {
    //ScanHeader[] scanHeaders = getScanHeaders();
    HashMap<Integer, ScanHeader> scanHeaderByInitialId = new HashMap<Integer, ScanHeader>();
   ...
   *
   * @param minRT the min rt
   * @param maxRT the max rt
   * @param msLevel the ms level
  ...
  public Integer[] getScanIdsForTimeRange(float minRT, float maxRT, int msLevel) throws SQLiteException {
    ...
    SQLiteStatement stmt = connection.prepare("SELECT id FROM scan WHERE ms_level = ? AND time >= ? AND time <= ?", true);
    stmt.bind(1, msLevel);
    stmt.bind(2, minRT);
    stmt.bind(3, maxRT);
   ...
   * @throws SQLiteException the sQ lite exception
   */
  /*public Integer[] getLazyScanIdsForTimeRange(float minRT, float maxRT, int msLevel) throws SQLiteException {
    int idmin = this.getScanHeaderForTime(minRT, msLevel).getId();
    int idmax = this.getScanHeaderForTime(maxRT, msLevel).getId();
    List<Integer> l = new ArrayList<Integer>();
    
    for (int i = idmin; i <= idmax; ++i) {
      if (this.getScanHeader(i).getMsLevel() == msLevel)
        l.add(i);
    }
