...
public abstract class AbstractScanSliceIterator {
   ...
   protected final SQLiteStatement statement;
	 ...
	 public AbstractScanSliceIterator(MzDbReader mzdbReader, String sqlQuery, int msLevel) throws SQLiteException {
		// super(); 
	   this.mzDbReader = mzdbReader;
	   
		 SQLiteConnection conn = mzDbReader.getConnection();
		 SQLiteStatement stmt = conn.prepare(sqlQuery, true); // false = disable statement cache
		 stmt.bind(1, msLevel);
		 
		 this.boundingBoxIterator = new BoundingBoxIterator(mzDbReader, stmt, mzdbReader.getDataEncodingByScanId(), msLevel);
		 this.statement = stmt;
