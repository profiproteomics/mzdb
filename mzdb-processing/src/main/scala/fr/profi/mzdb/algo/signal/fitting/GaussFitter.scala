  ...
  def value(variables: Array[Double]): Array[Double] = {
    ...
    for (i <- 0 until v.length) {
      ...
      for (j <- 0 until peaks.length) {
        ...
        if (x(i) < c)
          calcInt += paramGauss.value(x(i), Array[Double](a, b, c, d))
        else
          calcInt += paramGauss.value(x(i), Array[Double](a, b, c, e))
      }
      v(i) = calcInt
    }
    return v
  ...
  def jacobian(variables: Array[Double]): Array[Array[Double]] = {
    ...
    for (i <- 0 until x.length) {
      for (j <- 0 until peaks.length) {
        ...
        if (x(i) < c) {
          params = params ++ paramGauss.gradient(x(i), Array[Double](a, b, c, d))
          params += 0.0
        } else {
          params = params ++ paramGauss.gradient(x(i), Array[Double](a, b, c, e))
          params.insert(3, 0.0)
        }
