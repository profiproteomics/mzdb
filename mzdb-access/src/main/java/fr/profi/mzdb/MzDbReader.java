...
import fr.profi.mzdb.io.reader.InstrumentConfigReader;
import fr.profi.mzdb.io.reader.MzDbEntityCache;
import fr.profi.mzdb.io.reader.MzDbHeaderReader;
import fr.profi.mzdb.io.reader.RunReader;
import fr.profi.mzdb.io.reader.RunSliceHeaderReader;
import fr.profi.mzdb.io.reader.SampleReader;
import fr.profi.mzdb.io.reader.ScanHeaderReader;
import fr.profi.mzdb.io.reader.SoftwareReader;
import fr.profi.mzdb.io.reader.SourceFileReader;
...
public class MzDbReader {
	...
	this._paramNameGetter = (pwizMzDbVersion.compareTo("0.9.1") > 0) ? new MzDBParamName_0_9()
	...
	return new SQLiteQuery(this.connection, sqlString).extractSingleString();
    }
	...
	return this.getPwizMzDbVersion();
    }
	...
	this.bbSizes.BB_MZ_HEIGHT_MS1 = Double.parseDouble(
		this.mzDbHeader.getUserParam(paramNameGetter.getMs1BBMzWidthParamName()).getValue());
	this.bbSizes.BB_MZ_HEIGHT_MSn = Double.parseDouble(
		this.mzDbHeader.getUserParam(paramNameGetter.getMsnBBMzWidthParamName()).getValue());
	this.bbSizes.BB_RT_WIDTH_MS1 = Float.parseFloat(
		this.mzDbHeader.getUserParam(paramNameGetter.getMs1BBTimeWidthParamName()).getValue());
	this.bbSizes.BB_RT_WIDTH_MSn = Float.parseFloat(
		this.mzDbHeader.getUserParam(paramNameGetter.getMs1BBTimeWidthParamName()).getValue());
    }
	...
	return (float) new SQLiteQuery(this.connection, sqlString).extractSingleDouble();
    }
	...
	return new SQLiteQuery(this.connection, "SELECT max(ms_level) FROM run_slice").extractSingleInt();
    }
	...
	final SQLiteStatement stmt = this.connection
	...
	return new SQLiteQuery(this.connection, queryStr).bind(1, runSliceId).extractSingleInt();
    }
	...
	return new SQLiteQuery(this.connection, queryStr).extractSingleInt();
    }
	...
	return new SQLiteQuery(this.connection, queryStr).bind(1, msLevel).extractSingleInt();
    }
	...
	return new SQLiteQuery(this.connection, "SELECT seq FROM sqlite_sequence WHERE name = ?")
	...
	if ((this.diaIsolationWindows == null)) {
	    ...
	    this.diaIsolationWindows = isolationWindowList
		    .toArray(new IsolationWindow[isolationWindowList.size()]);
	}
	...
	String queryStr = "SELECT * FROM bounding_box" + " WHERE run_slice_id = ?"
		+ " ORDER BY first_spectrum_id"; // number

	// SQLiteStatement stmt =
	// connection.prepare("SELECT * FROM run_slice WHERE ms_level="+msLevel+" ORDER BY begin_mz ",
	// false);//number ASC", false);
	SQLiteRecordIterator records = new SQLiteQuery(this.connection, queryStr).bind(1, runSliceId)
		.getRecords();
	...
	while (records.hasNext()) {
	    ...
	    BoundingBox bb = BoundingBoxBuilder.buildBB(bbId, data, firstScanId, lastScanId, scanHeaderById,
	...
	return new SQLiteQuery(this.connection, sqlString).bind(1, bbId).extractSingleBlob();
    }
	...
	return new SQLiteQuery(this.connection, sqlString).bind(1, scanId).extractSingleLong();
    }
	...
	return (float) new SQLiteQuery(this.connection, sqlString).bind(1, bbId).extractSingleDouble();
    }
	...
	int runSliceId = new SQLiteQuery(this.connection, sqlString1).bind(1, bbId).extractSingleInt();

	String sqlString2 = "SELECT ms_level FROM run_slice WHERE run_slice.id = ?";
	return new SQLiteQuery(this.connection, sqlString2).bind(1, runSliceId).extractSingleInt();
	...
	SQLiteRecordIterator records = new SQLiteQuery(this.connection, sqlString).bind(1, firstScanId)
		.getRecords();
	...
	while (records.hasNext()) {
	    ...
	    BoundingBox bb = BoundingBoxBuilder.buildBB(r.columnInt(BoundingBoxTable.ID),
		    r.columnBlob(BoundingBoxTable.DATA), firstScanId, lastScanId, scanHeaderById,
		    dataEncodingByScanId);
	...
	for (BoundingBox bb : bbS) {
	    ...
	    /*
	     * System.out.println("searching for " +scanId); ScanSlice[] ssList =
	     * bbReader.readAllScanSlices(bb.getRunSliceId()); System.out.println("ssList.length: "
	     * +ssList.length); for( ScanSlice ss: ssList ) { System.out.println("has scan id="
	     * +ss.getScanId()); }
	     */
     ...
     */
    @Deprecated
    public ScanSlice[] getScanSlices(double minMz, double maxMz, float minRt, float maxRt, int msLevel)
	    throws SQLiteException, StreamCorruptedException {
	return this._getScanSlicesInRanges(minMz, maxMz, minRt, maxRt, msLevel, 0.0);
    }
     ...
     * @throws StreamCorruptedException
     */
    public ScanSlice[] getMsScanSlices(double minMz, double maxMz, float minRt, float maxRt)
	    throws SQLiteException, StreamCorruptedException {
	return this._getScanSlicesInRanges(minMz, maxMz, minRt, maxRt, 1, 0.0);
    }

    // TODO: think about msLevel > 2
    public ScanSlice[] getMsnScanSlices(double parentMz, double minMz, double maxMz, float minRt, float maxRt)
	    throws SQLiteException, StreamCorruptedException {
	return this._getScanSlicesInRanges(minMz, maxMz, minRt, maxRt, 2, parentMz);
    }

    private ScanSlice[] _getScanSlicesInRanges(double minMz, double maxMz, float minRt, float maxRt,
	    int msLevel, double parentMz) throws SQLiteException, StreamCorruptedException {
	...
	if (msLevel == 1) {
	    ...
	    sqliteQuery = new SQLiteQuery(this.connection, sqlQuery, false).bind(1, _minMz).bind(2, _maxMz)
		    .bind(3, _minRt).bind(4, _maxRt);

	} else {
	    String sqlQuery = "SELECT * FROM bounding_box WHERE id IN "
		    + "(SELECT id FROM bounding_box_msn_rtree" + " WHERE min_ms_level = " + msLevel
		    + " AND max_ms_level = " + msLevel + " AND min_parent_mz <= ? AND max_parent_mz >= ? "
		    + " AND min_mz >= ? AND max_mz <= ? AND min_time >= ? AND max_time <= ? )"
		    + " ORDER BY first_spectrum_id";

	    sqliteQuery = new SQLiteQuery(this.connection, sqlQuery, false).bind(1, parentMz)
		    .bind(2, parentMz).bind(3, _minMz).bind(4, _maxMz).bind(5, _minRt).bind(6, _maxRt);
	}
	...
	if (msLevel == 1) {
	    scanHeaderById = this.getMs1ScanHeaderById();
	} else if (msLevel == 2) {
	    scanHeaderById = this.getMs2ScanHeaderById();
	} else {
	...
	while (recordIter.hasNext()) {
		...
		if ((currentRt >= minRt) && (currentRt <= maxRt)) {
			...
			peaksCountByScanId.put(scanId,
				peaksCountByScanId.get(scanId) + scanSliceData.getPeaksCount());
		    }
		}
	    }
	}
	...
	for (Map.Entry<Long, ArrayList<ScanData>> entry : scanDataListById.entrySet()) {
	    ...
	    ScanData finalScanData = new ScanData(finalMzList, finalIntensityList, finalLeftHwhmList,
		    finalRightHwhmList);
	...
	SQLiteStatement stmt = this.connection.prepare(
     ...
     * @throws StreamCorruptedException
     */
    public Iterator<RunSlice> getLcMsRunSliceIterator(double minRunSliceMz, double maxRunSliceMz)
	    throws SQLiteException, StreamCorruptedException {
     ...
     */
    public Iterator<RunSlice> getLcMsnRunSliceIterator(double minParentMz, double maxParentMz,
	    double minRunSliceMz, double maxRunSliceMz) throws SQLiteException, StreamCorruptedException {
     ...
     *             the sQ lite exception
     */
    public Peak[] getXIC(double minMz, double maxMz, int msLevel, XicMethod method)
	    throws SQLiteException, StreamCorruptedException {
	return this.getXIC(minMz, maxMz, -1, -1, msLevel, method);
    }
	...
	return this.getXICForMz(mzCenter, mzTolInDa, minRt, maxRt, msLevel, method);
    }

    public Peak[] getXICForMz(double mz, double mzTolInDa, float minRt, float maxRt, int msLevel,
	...
	ScanSlice[] scanSlices = this.getMsScanSlices(minMz, maxMz, minRtForRtree, maxRtForRtree);

	final double mzTolPPM = MsUtils.DaToPPM(mz, mzTolInDa);
	return this._scanSlicesToXIC(scanSlices, mz, mzTolPPM, method);
    }

    public Peak[] getMsnXIC(double parentMz, double fragmentMz, double fragmentMzTolInDa, float minRt,
	    float maxRt, XicMethod method) throws SQLiteException, StreamCorruptedException {
	...
	ScanSlice[] scanSlices = this.getMsnScanSlices(parentMz, minFragMz, maxFragMz, minRtForRtree,
		maxRtForRtree);

	final double fragMzTolPPM = MsUtils.DaToPPM(fragmentMz, fragmentMzTolInDa);
	return this._scanSlicesToXIC(scanSlices, fragmentMz, fragMzTolPPM, method);
    }

    private Peak[] _scanSlicesToXIC(ScanSlice[] scanSlices, double searchedMz, double mzTolPPM,
	    XicMethod method) throws SQLiteException, StreamCorruptedException {
	...
	case NEAREST: {
		...
		if (nearestPeak == null) {
		    this.logger.error("nearest peak is null but should not be: searchedMz=" + searchedMz
			    + " minMz=" + slData.getMzList()[0] + " tol=" + mzTolPPM);
		    continue;
		}
	...
	case SUM: {
		...
		xicPeaks.add(new Peak(refPeak.getMz(), sum, refPeak.getLeftHwhm(), refPeak.getRightHwhm(),
	...
	default: {
	    this.logger.error(
		    "[_scanSlicesToXIC]: method must be one of 'MAX', 'NEAREST' or 'SUM', returning null");
	    return null;
	}
	}
