...
/***
 * This class allows to access to a mzDB file and to make some range queries on it.
 * A list of putative features can be provided to extract their corresponding signal.
 * 
 * @version 0.1
 * 
...
public class MzDbAccess {
	...
	*/
    ...
    //options.addOption("peakels_count", true, "number of feature eluting peaks");
  ...
  public static int _binSearchIndexToNearestIndex( int binSearchIndex, int length ) {
	...
	public static void main(String[] args) throws SQLiteException {
		...
		/*double[] values = {10.0,40.0,50.0};
    int minBinSearchIndex = Arrays.binarySearch(values, 9.0);
    int minIdx = _binSearchIndexToNearestIndex(minBinSearchIndex,values.length);
      ...
      e.printStackTrace();
        ...
        Peak[] peaks = mzDbInstance.getPeaks( min_mz, max_mz, min_time, max_time, 1);
		  ...
		  if( peaks != null ) {
			  ...
			  for( Peak peak : peaks ) { 
				  println( peak.getMz() + "\t" + peak.getIntensity() + "\t" + peak.getLeftHwhm()+ "\t" + peak.getRightHwhm() );
